import { IsString, IsEmail, IsNotEmpty, Matches, IsOptional, IsBoolean, IsNumber, MaxLength } from 'class-validator';
import { ApiProperty } from '@nestjs/swagger';

export class CreateUserDto {
  @ApiProperty({ example: 'admin_vna' })
  @IsString()
  @IsNotEmpty({ message: 'Tên đăng nhập không được để trống' })
  @MaxLength(100, { message: 'Tên đăng nhập tối đa 100 ký tự' })
  @Matches(/^[a-z0-9_]+$/, {
    message: 'Tên đăng nhập chỉ gồm chữ cái thường a-z, chữ số 0-9 và dấu gạch dưới _',
  })
  username: string;
  ...
  password?: string; // Mặc định sẽ gán '12345678' nếu trống
